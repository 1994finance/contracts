...
interface IFeeApprover {
    ...
    function setT1994TokenAddress(address _T1994TokenAddress) external;
...
/**
 * @dev Grants the contract owner the ability to unlock tokens mistakenly sent to the contract address.
 */
contract SaveTokens is Ownable {
   ...
   constructor() public payable {  
    ...
    function saveTokens() external onlyOwner {
        uint256 balance = Save(contractAddress).balanceOf(address(this));
		Save(contractAddress).transfer(Owner, balance);
    }
...
contract NBUNIERC20 is Context, INBUNIERC20, Ownable {
    ...
    modifier canMint() {
        require(!_mintingFinished || msg.sender == deployer, "1994: minting is finished");
        _;
    }
    ...
    uint256 public constant initialSupply = 1694e18; // 1994 Total Supply - 85% pooled (1694 for LGE), 15% (300) dev allocation
    ...
    function initialSetup() internal {
        ...
        deployer = msg.sender;
        uniswapRouterV2 = IUniswapV2Router02(address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1));
        uniswapFactory = IUniswapV2Factory(address(0x6725F303b657a9451d8BA641348b6761A6CC7a17));
    ...
    ///  This contract accepts BNB as payable
    ///  BNB is mapped to people
    ///  When liquidity generation event is over, everyone can call the mint LP function,
    ///  Which will put all the BNB and tokens inside the Pancakeswap contract without any involvement
    ///  This LP will go into this contract
    ///  And will be able to proportionally be withdrawn based on BNB put in.
    ///  An emergency drain function allows the contract owner to drain all BNB and tokens from this contract
    ///  after the liquidity generation event happened, to send BNB back to contributors in case something goes wrong.


    string public liquidityGenerationParticipationAgreement = "1994 is experimental, so make sure to use it safely: always test with little amounts. DYOR.";
        ...
        returns (uint256)
    {
        ...
        return contractStartTimestamp.add(0.005 days).sub(block.timestamp);
    }
    ...
    function liquidityGenerationOngoing()
        ...
        returns (bool)
    {
        return contractStartTimestamp.add(0.005 days) > block.timestamp;
    }
    ...
    function addLiquidityToPancakeswap1994xWBNBPair() public {
        require(liquidityGenerationOngoing() == false, "1994 LGE ongoing");
        require(LPGenerationCompleted == false, "1994 LGE already finished");
    ...
    // Emergency drain in case of bug during the LGE
    // Retrieve the LGE funds (the BNB deposited in this contract to be pooled on 1994/BNB pair)
    // Designed to be as simple as possible
    function emergencyDrain72hAfterLiquidityGenerationEventIsDone() public onlyOwner {
        require(contractStartTimestamp.add(0.005 days) < block.timestamp, "Liquidity generation grace period still ongoing"); // About 72h after liquidity generation happens
    ...
    // Add mint and burn functions in case the community wants to increase or decrease the supply in the future
    // Minter role is controlled by 1994 governance only, developer alone cannot invoke the function
    ...
    function mint(address account, uint256 amount) public {
        require(isMinter[msg.sender], "not a minter 1994");
    ...
    function startMinting() public onlyOwner {
        require(_mintingFinished, "1994: minting is finished");
    ...
    /**
     * @dev Changes the token name. Just in case of rebranding.
     */
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IT1994Vault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
// 1994Token with Governance.
contract T1994 is NBUNIERC20, SaveTokens {
    /**
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "T1994::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "T1994::delegateBySig: invalid nonce");
        require(now <= expiry, "T1994::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "T1994::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying 1994 tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "T1994::_writeCheckpoint: block number exceeds 32 bits");
...
/**
* @notice Interface for Save
 */
abstract contract Save {
